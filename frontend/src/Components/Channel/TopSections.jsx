        ...
        <div className=' text-white'>
            ...
            <div className='flex justify-between items-center'>
                <div className='flex my-10 gap-5'>
                    <div>
                        <img src={userData?.avatar} alt='avatarv image' className='rounded-full w-52 h-52' loading='lazy' />
                    </div>
                    <div className='flex flex-col justify-center '>
                        <h2 className='text-5xl font-bold z-50' >{userData?.fullName}</h2>
                        <p className='my-3 font-semibold text-white text-opacity-25'><span>{userData?.username} | </span>
                            <span>{userData?.subscriberCount}  Subscriber{userData?.subscriberCount !== 1 ? 's' : ''}</span></p>

                        <button onClick={() => handleSubcribeClick(userData._id)} className={`outline-none cursor-pointer rounded-full px-5 py-1 text-xl font-semibold border-2 border-orange-600 ${userData?.isSubscribed ? 'bg-neutral-800' : ''}`}>{userData?.isSubscribed ? 'Subscribed' : 'Subscribe'}</button>
                    </div>
                </div>
                <div>
                    <p className='border  mr-5 rounded-lg px-5 py-2 cursor-pointer hover:border-orange-700 hover:bg-white hover:bg-opacity-10 ' 
